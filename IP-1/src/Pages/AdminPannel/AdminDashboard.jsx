...
import './AdminPannel.css'; // Page-specific CSS
...
const initialPlatformStats = {
    ...
    totalProviders: 280,
    totalBookings: 3500,
    pendingApprovals: 15, // This will be updated by useEffect based on providers list
    ...
    revenueBreakdown: [
        ...
        // Corrected line: Use the monthlyRevenue value defined in this object, or a static fallback.
        { month: '2024-04', revenue: 750000, bookings: 360, newUsers: 190, newProviders: 15 },
    ]
};

// --- Mock Data for Management Lists (Initial State) ---
...
const AdminDashboardPage = () => {
    ...
    useEffect(() => {
        ...
        // Update platformStats with the new pendingCount
        // It's important to create a new object for state updates to ensure React detects the change.
        setPlatformStats(prevStats => ({
            ...prevStats, // Spread previous stats
            pendingApprovals: pendingCount, // Update only the pendingApprovals
            // Update the revenueBreakdown for April if monthlyRevenue has changed in state (though it's static here)
            // This ensures if monthlyRevenue becomes dynamic, the breakdown reflects it.
            revenueBreakdown: prevStats.revenueBreakdown.map(item =>
                item.month === '2024-04' ? { ...item, revenue: prevStats.monthlyRevenue } : item
            )
        }));
    }, [providers]); // Rerun when providers list changes
